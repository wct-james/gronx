...
func TestIsDue(t *testing.T) {
	...
	t.Run("minutes precision", func(t *testing.T) {
		expr := "*/2 * * * *"
		ref, _ := time.Parse(CronDateFormat, "2020-02-02 02:02")
		due, err := gron.IsDue(expr, ref)
		if err != nil {
			t.Errorf(err.Error())
		}
		if !due {
			next, _ := NextTickAfter(expr, ref, true)
			t.Errorf("%s should be due on %s, next: %s", expr, ref, next)
		}

		due, _ = gron.IsDue(expr, ref.Add(time.Minute))
		if due {
			next, _ := NextTickAfter(expr, ref, true)
			t.Errorf("time added, %s should be due on %s, next: %s", expr, ref, next)
		}
	})
...
func testcases() []Case {
	return []Case{
		...
		{"0 12 ? * *", "2020-08-20 00:00:00", false, "2020-08-20 12:00:00"},
		{"* ? * ? *", "2020-08-20 00:00:00", true, "2020-08-20 00:01:00"},
